...
def train(config_path: str, data_dir: str | None = None, demo_message: str | None = None, output_directory: str | None = None) -> Path:
    ...
    manifest = {"schema_version": "1.0", "paper_slug": "hidden", "paper": "HiDDeN", "run_kind": "tiny-set-overfit" if fixed_demo else ("dataset" if image_paths else "smoke"), "kind": "tiny-set-overfit" if fixed_demo else ("dataset" if image_paths else "smoke"), "status": "completed", "local_adapter": True, "data_directory": data_root, "demo_message": selected_demo_message if fixed_demo else None, "config": values, "history": history, "checkpoint": str(checkpoint), "device": str(device), "steps": len(history), "bit_error_rate": None, "artifacts": ["checkpoint.pt", "cover.png", "manifest.json"], "note": "Fixed one-image demo. It validates learnability, not generalisation or paper reproduction." if fixed_demo else ("Synthetic smoke run. Not a paper reproduction." if not image_paths else "Local dataset run. Match paper settings before comparing results." )}
