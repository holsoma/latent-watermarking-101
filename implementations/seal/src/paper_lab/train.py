...
def main():
    ...
    renderer=LocalDiffusionAdapter(config); noise=torch.randn(1,config.latent_channels,config.latent_size,config.latent_size); base_image=renderer.render(noise); key=key or semantic_key(base_image); marked=embed_noise(noise,config,key); image=renderer.render(marked); score=score_noise(renderer.invert(image),config,key)
