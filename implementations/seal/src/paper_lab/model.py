...
def semantic_key(image: torch.Tensor) -> str:
    """Create a stable, database-free key from coarse image semantics."""
    if image.ndim == 3: image = image.unsqueeze(0)
    pooled = F.adaptive_avg_pool2d(image, (2, 2)).mean(dim=1).flatten()
    stats = torch.cat((pooled, image.mean(dim=(-2, -1)).flatten(), image.std(dim=(-2, -1)).flatten()))
    # Coarse quantisation is intentional: the watermark must not change the
    # semantic key merely because a few low-level pixels moved.
    buckets = torch.clamp(((stats + 1) * 2).round(), 0, 4).to(torch.int64)
    return "".join(format(int(value), "x") for value in buckets)
