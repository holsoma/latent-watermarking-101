...
An optional `python -m rosteals_lab.official --model-id <model> --checkpoint outputs/demo/checkpoint.pt --image <image.png> --message <bits>` command runs the trained local offset and extractor through a pretrained Diffusers VAE. It rejects incompatible latent shapes and reports the recovered bits. It does not claim to load the official LaWa modified-decoder checkpoint.
