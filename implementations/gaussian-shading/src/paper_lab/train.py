...
def main():
    ...
    ber=bit_error(message,recovered); manifest={"schema_version":"1.0","paper_slug":"gaussian-shading","run_kind":"initial-noise-gaussian-shading","status":"completed","local_adapter":True,"implementation_fidelity":"mechanism-adapter","verification_status":"passed" if ber==0 else "failed","per_image_optimisation":False,"device":"cpu","seed":c.seed,"steps":0,"message":message,"recovered":recovered,"bit_error_rate":ber,"artifacts":["checkpoint.pt","base.png","watermarked.png","residual.png","manifest.json"]}; (output/"manifest.json").write_text(json.dumps(manifest,indent=2),encoding="utf-8"); print(json.dumps(manifest,indent=2))
