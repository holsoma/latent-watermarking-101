# Tree-Rings implementation lab

This lab follows the paper's initial-noise Fourier path: a key-dependent pattern is inserted into selected Fourier rings, a frozen diffusion renderer produces the image, and inversion tests the recovered spectrum. The default run is a deterministic reversible adapter for CPU tests. The module also exposes an optional Diffusers loader for an actual Stable Diffusion pipeline when model weights and the `diffusers` package are available.

Official implementation: https://github.com/YuxinWenRick/tree-ring-watermark

```bash
cd /c/amos/research/latent-watermarking-101/implementations/tree-rings
