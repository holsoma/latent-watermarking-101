...
def main():
    ...
    renderer=LocalDiffusionAdapter(config); noise=torch.randn(1,config.latent_channels,config.latent_size,config.latent_size); marked=embed_noise(noise,config,key); image=renderer.render(marked); recovered_noise=renderer.invert(image); score=score_noise(recovered_noise,config,key); wrong_score=score_noise(recovered_noise,config,"null:"+key); verified=score>wrong_score
    ...
    manifest={"schema_version":"1.0","paper_slug":"tree-rings","run_kind":"initial-noise-fourier-mark","status":"completed","local_adapter":True,"implementation_fidelity":"mechanism-adapter","verification_status":"passed" if verified else "failed","per_image_optimisation":False,"device":"cpu","seed":config.seed,"steps":0,"message":key,"recovered":key if verified else None,"score":score,"negative_control_score":wrong_score,"score_margin":score-wrong_score,"bit_error_rate":None,"artifacts":["checkpoint.pt","base.png","watermarked.png","residual.png","manifest.json"]}
