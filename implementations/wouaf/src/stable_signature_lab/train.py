...
def train(args):
    ...
    manifest = {"schema_version": "1.0", "paper_slug": "wouaf", "method": "wouaf", "run_kind": "fingerprint-conditioned-weight-modulation", "status": "completed", "local_adapter": True, "device": str(device), "seed": int(values.get("seed", 23)), "steps": int(values.get("steps", 700)), "key": key_text, "recovered": recovered, "bit_error_rate": errors / config.message_length, "artifacts": ["checkpoint.pt", "base_decoder.png", "marked_decoder.png", "residual_amplified.png", "manifest.json"], "history": history, "checkpoint": str(checkpoint), "note": "The local adapter trains rank-limited latent adapters; the paper modulates generator weights with user fingerprints."}
