...
def train(config_path: str, output_directory: str | None = None) -> Path:
    ...
    manifest = {"paper": "RoSteALS", "kind": "tiny-set-random-message", "config": values, "checkpoint": str(output / "checkpoint.pt"), "device": str(device), "message_length": config.message_length, "test_message": "".join(str(int(bit)) for bit in target[0].tolist()), "test_recovered": "".join(str(int(bit)) for bit in predicted[0].tolist()), "bit_error_rate": ber, "image_mse_vs_autoencoder": float(((encoded - model.autoencoder.decode(model.autoencoder.encode(last_cover))) ** 2).mean().item()), "history": history, "note": "Frozen analytic autoencoder adapter for a local mechanism demo. Replace it with the official VQ-f4 checkpoint for paper-matched work."}
